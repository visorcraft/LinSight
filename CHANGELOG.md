...
- **Multi-host view.** Save remote hosts in `~/.config/linsight/hosts.json`
  ...
  `ssh://` hosts, and "Manage…"). The GUI sample stream survives reconnects:
  switching hosts rebuilds the dashboard tiles from the new daemon's sensor
  catalogue, replays subscriptions and the pump interval, and keeps the
  previous connection alive if the reconnect fails. Hosts are validated as
  unique, non-empty names with `ssh://` URLs. Launch straight to a host with
  `linsight --connect ssh://user@host`. Key-based SSH auth is assumed, with
  the same trust model as `ssh -L`.
...
- **Alert event log and cooldown.** Every alert fire and clear is recorded
  in a ring buffer (512 events) visible on the Alerts page. Per-rule
  cooldown suppresses re-notification for a configurable window after a
  fire, preventing flapping alerts from spamming notify targets. Cooldown
  is editable in the GUI rule editor and via the CLI `--cooldown` flag.
